...
%% Weights
% Monkey randomly picks a stock numpicks times, that stock is added
% 1/numpicks weighted to the index. A stock can be picked multiple times,
% increasing its weight

weights = zeros(size(MCData));
numpicks = 1000;

for year = 1:size(MCData,1)
    for picks = 1:numpicks
        pos = randi(length(LargComps));
        monkey_pick = LargComps(year,pos);
        weights(year,monkey_pick) = weights(year,monkey_pick) + (1/numpicks);
    end
end
...
RandInvest = zeros(size(sum_returns,1),1);
RandInvest(1) = 1;
for i = 1:size(sum_returns,1)
    RandInvest(i+1) = RandInvest(i) * (1+sum_returns(i));
end
...
avr_ann_return = ((RandInvest(end))^(1/numyears) -1);
...
% Maximum Drawdown
maxdraw = maxdrawdown(RandInvest);
...
Randaveragesize2018 = mean(weighted_sizes);
...
% Results Vector
Randresults = [avr_ann_return*100 ann_vol*100 sharpe maxdraw*100 pos_months turnover*100];
Randexresults = [ret_diff*100 tracking_error*100 information_ratio];
...
Rand_recent_results = [avr_recent_return*100 recent_vol*100 rec_sharpe];

%% Save Data for Total.m

save("Data/RandInvest.mat","RandInvest","Randresults","Randexresults","Rand_recent_results","Randaveragesize2018");
