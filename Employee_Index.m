...
% importEmpactive = xlsread("Data/Raw/Emps_active.xlsx");
% importEmpdeadRCI = xlsread("Data/Raw/Emps_deadRCIY.xlsx");
% importEmpdeadRCI = [importEmpdeadRCI,zeros(size(importEmpdeadRCI,1),7)];
% importEmpdeadnoRCI= xlsread("Data/Raw/Emps_deadRCIN.xlsx");
% save("Data/importedEmpData.mat","importEmpactive","importEmpdeadRCI","importEmpdeadnoRCI"); 

load("Data/importedEmpData.mat");

%% Matrices

load("Data/PricesData.mat");
EmpData = [importEmpactive(:,2:end),importEmpdeadRCI(:,2:end),importEmpdeadnoRCI(:,2:end-1)];
save("Data/EmpData.mat","EmpData");
...
%% Cleaning
EmpData(isnan(EmpData))=0;

%% Weights
weights = zeros(size(EmpData));

for i = 1:size(EmpData,1)
total = sum(EmpData(i,LargComps(i,:)));
weights(i,LargComps(i,:)) = EmpData(i,LargComps(i,:))./total;
end
...
EmpInvest = zeros(size(sum_returns,1),1);
EmpInvest(1) = 1;
for i = 1:size(sum_returns,1)
    EmpInvest(i+1) = EmpInvest(i) * (1+sum_returns(i));
end
...
avr_ann_return = ((EmpInvest(end))^(1/numyears) -1);
...
Empaveragesize2018 = mean(weighted_sizes);
...
% Results Vector
Empresults = [avr_ann_return*100 ann_vol*100 sharpe maxdraw*100 pos_months turnover*100];
Empexresults = [ret_diff*100 tracking_error*100 information_ratio];
...
Emp_recent_results = [avr_recent_return*100 recent_vol*100 rec_sharpe];

%% Save Data for Total.m

save("Data/EmpInvest.mat","EmpInvest","Empresults","Empexresults","Emp_recent_results","Empaveragesize2018");
