...
weights(i,LargComps(i,:)) = 1/numStocks;
end
...
EqInvest = zeros(size(sum_returns,1),1);
EqInvest(1) = 1;
for i = 1:size(sum_returns,1)
    EqInvest(i+1) = EqInvest(i) * (1+sum_returns(i));
end
...
avr_ann_return = ((EqInvest(end))^(1/numyears) -1);
...
Eqaveragesize2018 = mean(weighted_sizes);
...
% Results Vector
Eqresults = [avr_ann_return*100 ann_vol*100 sharpe maxdraw*100 pos_months turnover*100];
Eqexresults = [ret_diff*100 tracking_error*100 information_ratio];
...
Eq_recent_results = [avr_recent_return*100 recent_vol*100 rec_sharpe];

%% Save Data for Total.m

save("Data/EqInvest.mat","EqInvest","Eqresults","Eqexresults","Eq_recent_results","Eqaveragesize2018");
