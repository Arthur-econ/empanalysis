...
% importDivactive = xlsread("Data/Raw/Div_active.xlsx");
% importDivdeadRCI = xlsread("Data/Raw/Div_deadRCIY.xlsx");
% importDivdeadRCI = [importDivdeadRCI,zeros(size(importDivdeadRCI,1),2)];
% importDivdeadnoRCI = xlsread("Data/Raw/Div_deadRCIN.xlsx");
% save("Data/importedDivData.mat","importDivactive","importDivdeadRCI","importDivdeadnoRCI"); 

load("Data/importedDivData.mat");


%% Matrices

load("Data/PricesData.mat");
DivData = [importDivactive(:,2:end),importDivdeadRCI(:,2:end),importDivdeadnoRCI(:,2:end-1)];
save("Data/DivData.mat","DivData");
...
%% Cleaning
DivData(isnan(DivData))=0;

%% Weights
weights = zeros(size(DivData));

for i = 1:size(DivData,1)
total = sum(DivData(i,LargComps(i,:)));
weights(i,LargComps(i,:)) = DivData(i,LargComps(i,:))./total;
end
...
DivInvest = zeros(size(sum_returns,1),1);
DivInvest(1) = 1;
for i = 1:size(sum_returns,1)
    DivInvest(i+1) = DivInvest(i) * (1+sum_returns(i));
end
...
avr_ann_return = ((DivInvest(end))^(1/numyears) -1);
...
Divaveragesize2018 = mean(weighted_sizes);
...
% Results Vector
Divresults = [avr_ann_return*100 ann_vol*100 sharpe maxdraw*100 pos_months turnover*100];
Divexresults = [ret_diff*100 tracking_error*100 information_ratio];
...
Div_recent_results = [avr_recent_return*100 recent_vol*100 rec_sharpe];

%% Save Data for Total.m

save("Data/DivInvest.mat","DivInvest","Divresults","Divexresults","Div_recent_results","Divaveragesize2018");
